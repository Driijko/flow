...
<script>
  ...
  // EVENT HANDLERS --------------------------
  function handleClickOG(level, name) {
  ...
  function handleClick(levelName, levelNum) {
    ...
    && $siteMenuStore.navigation.open[levelNum] !== levelName) {
      siteMenuStore.collapseNav(levelName);
    } else {
      scroll("right");
    }
  };
    ...
    {#each lists as list,listIndex}
      {#if listIndex === 0}
        <ul id="navigation0">
            {#each list as link}
              ...
              <li 
                class:selected={$siteMenuStore.navigation[listIndex] === link.tag}
              >
                <a href={link.tag} 
      ...
      {:else if $siteMenuStore.navigation.open[listIndex - 1] 
        ...
        <ul id="navigation{listIndex}">
          {#each list[map[$siteMenuStore.navigation.open[listIndex - 1]]] as link,linkIndex}
            <li 
              class:selected={
                $siteMenuStore.navigation.open[listIndex] === link.tag
              }
            >
              <a href={link.tag} 
                class:column={$viewportOrientationStore === "portrait"}
